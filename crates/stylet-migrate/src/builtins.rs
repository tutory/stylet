...
pub enum Result {
    ...
    Warn(Value, String),
    /// A Stylus built-in the migration doesn't implement: passed through as CSS.
    Unsupported(Value),
    /// Not a known function: passed through as CSS.
    Unknown(Value),
}

/// Stylus' built-in functions (those not implemented here are reported as
/// unsupported rather than as unknown JS functions).
const STYLUS_BUILTINS: &[&str] = &[
    "red",
    "green",
    "blue",
    "alpha",
...
pub fn call(name: &str, args: &[(Option<String>, Value)]) -> Result {
    ...
    Result::Value(match lower.as_str() {
        ...
        _ if STYLUS_BUILTINS.contains(&lower.as_str()) => return Result::Unsupported(passthrough()),
